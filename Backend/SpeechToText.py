...
#Initialize the Chrome wegdriver using the ChronellriverManager. 
service = Service(ChromeDriverManager().install()) 
# try:
driver = webdriver.Chrome(service=service, options=chrome_options)

    # Give time for browser to start
    # time.sleep(2)
...
#Function to set the assistant's status by writing it to a fe 
...
def QueryModifier(Query): 
    ...
    question_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whow", "can you", "what's", "where's", "how's", "can you"] 
    ...
    if any(word + " " in new_query for word in question_words): 
        ...
        #add a period if the query is out a question, 
        if query_words[-1][-1] in ['.', '?', '!']:
             new_query = new_query[:-1] + "." 
        else: 
            new_query  += "."
